...
it('retrieves an event with tags added without a specific position', function () {
    $db_file = tempnam(sys_get_temp_dir(), 'test') . '.sqlite';
    $sqlite = new SQLite3($db_file);
    expect($db_file)->toBeFile();
...
it('stores an event with tags', function () {
    ...
    $store = new nostriphant\Transpher\Stores\SQLite($sqlite, $subscription, Mockery::spy(\Psr\Log\LoggerInterface::class));
    expect($sqlite->lastErrorMsg())->toBe('not an error');
    ...
    expect($subscription($event))->toBeFalse();
...
it('ignores an event that matches ignore filter', function () {
    $db_file = tempnam(sys_get_temp_dir(), 'test') . '.sqlite';
    $sqlite = new SQLite3($db_file);
    expect($db_file)->toBeFile();

    $ignore = Subscription::make(['ids' => ['07cf455963bffe4ef851e4983df2d1495602714abc6c0e028c02752b16e11bcb']]);
    $store = new nostriphant\Transpher\Stores\SQLite($sqlite, $ignore, Mockery::spy(\Psr\Log\LoggerInterface::class));
    expect($sqlite->lastErrorMsg())->toBe('not an error');
...
it('deletes events matching ignore filter', function () {
    $db_file = tempnam(sys_get_temp_dir(), 'test') . '.sqlite';
    $sqlite = new SQLite3($db_file);
    expect($db_file)->toBeFile();
