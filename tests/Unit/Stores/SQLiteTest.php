...
it('creates a table `event` if not exists', function () {
    ...
    $store = new nostriphant\Transpher\Stores\SQLite($sqlite, Subscription::make([]));
...
it('stores an event with tags', function () {
    ...
    $store = new nostriphant\Transpher\Stores\SQLite($sqlite, $subscription);
    expect($sqlite->lastErrorMsg())->toBe('not an error');
    ...
    expect($subscription($event))->toBeNull();
