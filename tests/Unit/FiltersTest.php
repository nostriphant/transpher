...
it('filters p-tags', function() {
    $subscription = Filters::make(['#p' => ['RandomPTag']]);
...
it('filters e-tags', function() {
    $subscription = Filters::make(['#e' => ['RandomEventId']]);
...
it('filters created since', function() {
    $time = time();
    $subscription = Filters::make(['since' => $time - 100]);
...
it('filters created until', function() {
    $time = time();
    $subscription = Filters::make(['until' => $time + 100]);
...
it('filters maximum number of items', function() {
    $time = time();
    $subscription = Filters::make(['until' => $time + 100, 'limit' => 5]);
...
it('handlers multiple filters', function() {
    $subscription = Filters::make(['#p' => ['RandomPTag']], ['#p' => ['RandomPTag2']]);
