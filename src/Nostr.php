...
class Nostr {
    ...
    static function wrap(callable $respond) {
        return fn(array ...$messages) => \Functional\each($messages, fn(array $message) => $respond(self::encode($message)));
    }
    ...
    static function event(callable $private_key, int $created_at, int $kind, array $tags, string $content) {
